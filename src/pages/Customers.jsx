...
function Customers() {
  ...
  const filtered = useMemo(() => Customers.filter((c) => c?.name?.toLowerCase()?.includes(query?.toLowerCase()) || c?.phone.includes(Number(query))), [Customers, query])

  function handleAddCustomer(e) {
    e.preventDefault();
    if (!name.trim()) return;
    setCustomers((prev) => [...prev, { id: "C-" + crypto.randomUUID(), name, phone, address, balance: 0 }]);
    setPhone("");
    setAddress("");
    setName("");
    setShowForm(false);
  ...
  return (
    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setShowForm(prev => !prev)} className="flex items-center gap-2 bg-[#4F46E5] text-white font-medium text-sm px-4 py-2.5 rounded-xl shadow-[0_4px_14px_rgba(79,70,229,0.35)] hover:bg-[#4338CA] cursor-pointer transition">
      ...
      {showForm && (<form onSubmit={handleAddCustomer} className="bg-white border border-[#EDEEF2] rounded-2xl p-5 mb-6 flex flex-col sm:flex-row gap-3 shadow-[0_1px_2px_rgba(16,24,40,0.04)] ">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name" type="text" name="" id="" className="flex-1 bg-[#F7F8FB] border border-[#E5E7EB] rounded-xl px-3.5 py-2.5 text-sm text-[#111827] placeholder-[#9AA0AA] outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/15 transition"
          autoFocus />
        <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Address" type="text" name="" id="" className="flex-1 bg-[#F7F8FB] border border-[#E5E7EB] rounded-xl px-3.5 py-2.5 text-sm text-[#111827] placeholder-[#9AA0AA] outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/15 transition"
        />
        <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="Phone Number" type="tel" className="flex-1 bg-[#F7F8FB] border border-[#E5E7EB] rounded-xl px-3.5 py-2.5 text-sm text-[#111827] placeholder-[#9AA0AA] outline-none focus:border-[#4F46E5] focus:ring-2 focus:ring-[#4F46E5]/15 transition"
        />
